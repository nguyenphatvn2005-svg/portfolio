    ...
    <a href="project.html?id=${encodeURIComponent(project.id)}"
      ...
      <div class="absolute inset-x-0 bottom-0 p-7 md:p-9">
        <div class="mb-4 flex items-center justify-between gap-4">
          <span class="text-[11px] font-bold uppercase tracking-[.22em] text-[#d9b66f]">${escapeHTML(project.eyebrow)}</span>
          <span class="text-[11px] uppercase tracking-[.18em] text-white/45">0${index + 1}</span>
        </div>
        <h3 class="font-display text-3xl italic text-white md:text-4xl">${escapeHTML(project.title)}</h3>
        <p class="mt-3 max-w-md text-sm leading-6 text-white/65">${escapeHTML(project.shortDescription)}</p>
        <span class="mt-6 inline-flex items-center gap-2 text-sm font-semibold text-white">
          Xem case study <i data-lucide="arrow-up-right" class="arrow h-4 w-4 text-[#d9b66f]"></i>
        </span>
      </div>
    </a>
  `).join("");
...
function renderProjectPage() {
  ...
  const mediaSections = (project.mediaSections || []).map(section => {
      ...
      if (section.type === "video") {
        return `
          <figure class="media-card">
            ...
            <figcaption class="px-4 py-3 text-xs uppercase tracking-[.16em] text-white/45">${escapeHTML(item.label || "Video")}</figcaption>
          </figure>`;
      }
      const aspect = section.aspect === "portrait" ? "aspect-[4/5]" : "aspect-square";
      return `
        <figure class="media-card">
          <img src="${escapeHTML(item.src)}" alt="${escapeHTML(item.alt || project.title)}" loading="lazy" class="${aspect} w-full object-cover">
        </figure>`;
    }).join("");
  ...
  root.innerHTML = `
    <header class="relative min-h-[78vh] overflow-hidden border-b border-white/10 noise">
      <img src="${escapeHTML(project.heroImage)}" alt="" class="absolute inset-0 h-full w-full object-cover opacity-45">
      ...
      <div class="relative mx-auto flex min-h-[78vh] max-w-6xl items-end px-6 pb-16 pt-36 md:pb-20">
        <div class="max-w-4xl reveal">
          <a href="index.html#projects" class="mb-8 inline-flex items-center gap-2 text-sm text-white/60 transition hover:text-[#d9b66f]">
            <i data-lucide="arrow-left" class="h-4 w-4"></i> Tất cả dự án
          </a>
          <p class="text-xs font-bold uppercase tracking-[.3em] text-[#d9b66f]">${escapeHTML(project.eyebrow)} · Case Study</p>
          <h1 class="font-display mt-5 text-5xl italic leading-[.98] text-white sm:text-6xl md:text-8xl">${escapeHTML(project.title)}</h1>
          <p class="mt-6 max-w-2xl text-base leading-7 text-white/65 md:text-lg">${escapeHTML(project.heroDescription)}</p>
        </div>
      </div>
    </header>
      ...
      <section class="reveal border-t border-white/10 py-16 md:py-20">
        <div class="grid gap-12 md:grid-cols-2">
          <div>
            ...
            <ul class="mt-7 space-y-4">${project.roles.map(item => `<li class="flex gap-3 text-gray-400"><span class="mt-2 h-1.5 w-1.5 shrink-0 rounded-full bg-[#d9b66f]"></span><span>${escapeHTML(item)}</span></li>`).join("")}</ul>
          </div>
          <div>
            ...
            <ul class="mt-7 space-y-4">${project.results.map(item => `<li class="flex gap-3 text-gray-400"><span class="mt-2 h-1.5 w-1.5 shrink-0 rounded-full bg-[#d9b66f]"></span><span>${escapeHTML(item)}</span></li>`).join("")}</ul>
          </div>
        </div>
      </section>
        ...
        <div class="flex flex-col items-start justify-between gap-8 rounded-[2rem] border border-white/10 bg-white/[.025] p-7 md:flex-row md:items-end md:p-10">
          <div>
            ...
            <a class="mt-5 inline-flex items-center gap-2 text-sm font-semibold text-white hover:text-[#d9b66f]" href="project.html?id=${encodeURIComponent(nextProject.id)}">
              Xem dự án tiếp theo <i data-lucide="arrow-right" class="h-4 w-4"></i>
            </a>
          </div>
